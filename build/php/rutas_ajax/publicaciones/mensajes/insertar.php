<?php 
    ...
    $query = "INSERT INTO Publicaciones(usuario_creador_id,grupo_id,tipo,fecha_creacion) VALUES ($usuario,$grupo,'M',CURRENT_TIMESTAMP) RETURNING publicacion_id";
    ...
    if($result){   
        ...
        //creacion del evento
        $query = "INSERT INTO Mensajes(publicacion_id,informacion) VALUES ($publicacion,'$informacion') RETURNING mensaje_id";
        $result = pg_query($link, $query);
        if($result){    
            $rows = pg_fetch_assoc($result);
            $create = $rows["mensaje_id"];            
        }
    }
    echo $create
?>