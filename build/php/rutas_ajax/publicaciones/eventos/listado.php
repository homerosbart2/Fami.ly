<?php 
    ...
    if($result){
        ...
        $query = "SELECT A.asiste_id, A.estado, U.nombres, U.formato_img, U.name_img, U.usuario_id FROM  Asiste AS A, Usuarios AS U WHERE A.evento_id = $evento AND U.usuario_id = A.usuario_id";
        $result = pg_query($link, $query);
        $i = 0;
        if($result){
            while($row = pg_fetch_assoc($result)){
